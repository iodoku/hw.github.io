...
def applygetapplications(mysql):
    try:
        ...
        # 기본 쿼리 작성
        query = """
            SELECT id, user_id, job_id, application_date, status, notes, created_at
            FROM applications
            WHERE user_id = %s
        """
        params = [user_id]
...
def applydeleteapplications(mysql, id):
    try:
        ...
        if not application:
            response_data = {
                "status": "error",
                "message": "지원 내역을 찾을 수 없거나 권한이 없습니다.",
                "code": "404"
            }
